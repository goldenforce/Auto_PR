...
BEGIN
   str_SQL :=
    ...
    (SELECT CASE WHEN IRGU.GU_ID='''||  '01' || '''' || ' THEN ''' || 'SNAT' || '''' || ' WHEN IRGU.GU_ID <> ''' || 'US' || '''' || ' THEN ''' || 'NON-US' || '''' || ' ELSE ''' || 'US' || '''' || ' END FROM FT_T_ISSR ISSR, FT_T_IRGU IRGU WHERE ISSR.INSTR_ISSR_ID=I.INSTR_ISSR_ID AND ISSR.INSTR_ISSR_ID = IRGU.INSTR_ISSR_ID AND (ISSR.END_TMS IS NULL OR ISSR.END_TMS > CURRENT_TIMESTAMP) AND (IRGU.END_TMS IS NULL OR IRGU.END_TMS > CURRENT_TIMESTAMP) AND IRGU.ISSR_GU_PURP_TYP = '''
      || 'INCRPRTE'
      || ''''
      || ' and IRGU.GU_TYP = CASE WHEN gu_id='''|| '01' || '''' || ' THEN ''' || 'UNKNOWN' ||'''' || ' ELSE ''' || 'COUNTRY' || '''END'
