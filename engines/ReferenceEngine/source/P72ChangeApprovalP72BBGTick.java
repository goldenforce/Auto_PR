...
public class P72ChangeApprovalP72BBGTick implements JavaRule {
	...
	public static final String GET_ISID_OID = "SELECT ISID.ISID_OID FROM FT_T_MIXR MIXR1, FT_T_ISID ISID"
			...
			+ " AND ISID2.ID_CTXT_TYP = 'P72BBGTICK'"
			+ " AND (ISID2.END_TMS IS NULL OR ISID2.END_TMS >= SYSDATE())"
			+ " AND (MIXR2.END_TMS IS NULL OR MIXR2.END_TMS >= SYSDATE()))"
			+ " LIMIT 1";

	private static Logger logger = Logger.getLogger("P72ChangeApprovalP72BBGTick");
	SegmentId P72BBGTICKSegmentId = new SegmentId(0);
	...
	public boolean initialize(String[] param) 
	{
		...
		if (param_AllMsgClassifications != null && !("".equalsIgnoreCase(param_AllMsgClassifications) || " ".equalsIgnoreCase(param_AllMsgClassifications) || param_AllMsgClassifications == null)) 
		{
			...
			loggerDebug("P72ChangeApprovalP72BBGTick : initialize sucessfully initiallized Message Classification List = " + msgClassificationList);
		}
		else 
		{
			loggerDebug("P72ChangeApprovalP72BBGTick : initialize not initialized properly");
		}
	...
	public boolean process(XMLMessage msg, DatabaseObjectContainer dboc, ProcessorContext pContext, DatabaseAccess db, NotificationCreator notificationCreator) throws GSException {
		...
		loggerDebug("P72ChangeApprovalP72BBGTick : process Execution started");
		...
		loggerDebug("P72ChangeApprovalP72BBGTick : process curSeg = '" + msg.getSegmentType(curSeg) + "'");
		//loggerDebug("P72ChangeApprovalP72BBGTick : process segProcInd = '" + segProcInd + "'");
		loggerDebug("P72ChangeApprovalP72BBGTick : process msgClassification = '" + msgClassification + "'");
		...
		{
			loggerDebug("P72ChangeApprovalP72BBGTick : Message Classification does not require processing, hence exiting.");
			return true;
		}
		...
		{
			loggerDebug("P72ChangeApprovalP72BBGTick : Segment Type does not require processing, hence exiting.");
			return true;
		}
		...
		try
		{
			...
			//loggerDebug("P72ChangeApprovalP72BBGTick : Instrument ID from Message - " + instrId);

			if ("CAProcessing".equalsIgnoreCase(msgClassification))
			{
				String mainEntityId = msg.getStringField("MAIN_ENTITY_ID",new SegmentId(0));
				loggerDebug("P72ChangeApprovalP72BBGTick : Main Entity Id - " + mainEntityId);

				if (mainEntityId != null && !"".equalsIgnoreCase(mainEntityId))
				{
					for (int i = 0 ; i < msg.getSegmentCount() ; i++)
					{
						P72BBGTICKSegmentId = new SegmentId(i);
						String currSegName = msg.getSegmentType(P72BBGTICKSegmentId);

						loggerDebug("P72ChangeApprovalP72BBGTick : Segment Name ISID Loop - " + currSegName);

						if (("IssueIdentifier").equalsIgnoreCase(currSegName))
						{
							String idCtxtTyp = msg.getStringField("ID_CTXT_TYP", P72BBGTICKSegmentId);
							loggerDebug("P72ChangeApprovalP72BBGTick : ID Context : " + idCtxtTyp);

							if ("P72BBGTICK".equalsIgnoreCase(idCtxtTyp))
							{
		...
		{
			logger.error("P72ChangeApprovalP72BBGTick : Error while processing Rule ." + e.getMessage() + e.getStackTrace().toString());
			return false;
		}
		loggerDebug("P72ChangeApprovalP72BBGTick : XML Message : " + msg.getXMLString());
		loggerDebug("P72ChangeApprovalP72BBGTick : process Executed successfully");
		return true;
	}
	...
	public void addIsidOid (XMLMessage msg, String mktIssOid, DatabaseAccess db)
	{
		...
		loggerDebug("P72ChangeApprovalP72BBGTick : Inside ISID Oid function");
		try
		{
			...
			if (!db.isEndOfStream())
			{
				...
				loggerDebug("P72ChangeApprovalP72BBGTick : Isid oid from DB :" + isidOid);

				String P72BBGTICKIsidOidFromMsg = msg.getStringField("ISID_OID", P72BBGTICKSegmentId);

				if (P72BBGTICKIsidOidFromMsg != null && !"".equalsIgnoreCase(P72BBGTICKIsidOidFromMsg))
				{
					msg.setFieldValue("ISID_OID", P72BBGTICKSegmentId, isidOid);
				}
				else
				{
					msg.addField("ISID_OID", P72BBGTICKSegmentId, isidOid);
				}
			}
		}
		catch (Exception e) 
