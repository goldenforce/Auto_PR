...
public class CJavaP72ConvertNewISIDToUpdate implements JavaRule {
	private static Logger logger = Logger.getLogger("CJavaP72ConvertNewISIDToUpdate");
	...
	public boolean initialize(String[] param) {
		if (param.length < 2) {
			logger.error("Required Parameters missing for CJavaP72ConvertNewISIDToUpdate in StreetRefMsgSet ");
			return false;
		}
		if (logger.isDebugEnabled())
			logger.debug("CJavaP72ConvertNewISIDToUpdate | initialize | Started Initializing ....");
		...
		if (logger.isDebugEnabled())
			logger.debug("CJavaP72ConvertNewISIDToUpdate | initialize | Completed Initializing ....");
		return true;
	...
	public boolean process(XMLMessage msg, DatabaseObjectContainer dboc, ProcessorContext pContext,
		...
		logger.info("CJavaP72ConvertNewISIDToUpdate process start");
		...
		for (int i=0; i<idCtxtTypList.size(); i++)
		{
			...
			for (int j = 0; j < isidSegIdList.size(); j++) 
			{
				...
				String idCtxtTypPrev=idCtxtTyp+"_-_Previous";
				
				if (vendorMnem == null || "".equals(vendorMnem) || !idCtxtTypPrev.equals(vendorMnem)) 
				{
					if (logger.isDebugEnabled()) logger.debug("No VENDOR_MNEMONIC found in Segment..i.e. NEW "+idCtxtTyp);
						
					newIsidSegId=isidSegIdList.get(j);
					if ((commons.getStringFieldFromMsg(newIsidSegId, "ISS_ID").contains("^")) && "RIC".equals(idCtxtTyp))
					{
						if (logger.isDebugEnabled()) logger.debug("New RIC contains '^'..hence exiting..");
						break;
					}	
